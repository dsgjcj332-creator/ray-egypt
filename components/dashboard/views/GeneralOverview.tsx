...
const GeneralOverview: React.FC<GeneralOverviewProps> = ({ onSwitchType }) => {
  ...
  // Global Actions with IDs
  const globalActions = [
    { id: 'act_new_branch', label: 'إضافة فرع/نشاط', icon: Plus, action: 'new_branch' },
    { id: 'act_users', label: 'إدارة المستخدمين', icon: Users, action: 'users' },
    { id: 'act_reports', label: 'التقارير المجمعة', icon: FileText, action: 'reports' },
    { id: 'act_settings', label: 'إعدادات المنصة', icon: Settings, action: 'settings' },
    { id: 'act_audit', label: 'سجل النشاطات', icon: Globe, action: 'audit_log' },
    { id: 'act_perms', label: 'الصلاحيات', icon: Shield, action: 'permissions' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 p-4 md:p-8 pb-24 max-w-[1600px] mx-auto relative">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        {/* Welcome Banner */}
        <div className="xl:col-span-2 bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden flex flex-col justify-center min-h-[220px] group">
          ...
          <div className="absolute right-[-20px] bottom-[-20px] w-40 h-40 bg-blue-500/20 rounded-full blur-3xl"></div>
          
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-bold text-ray-gold mb-4 border border-white/10 shadow-sm">
              <LayoutDashboard className="w-3 h-3" />
              لوحة التحكم الرئيسية
            </div>
            <h1 className="text-3xl md:text-4xl font-black mb-3 tracking-tight">مركز القيادة الموحد (RAY Hub)</h1>
            <p className="text-slate-300 max-w-xl text-sm md:text-base leading-relaxed">
              مرحباً بك. هنا يمكنك متابعة أداء جميع أنشطتك التجارية، إدارة الفروع، والتحكم الكامل في النظام من مكان واحد.
            </p>
          </div>
        </div>
