...
import { 
  ...
  Scissors, Pill, Dumbbell, ShoppingBag, Tag, User, CheckCircle, Loader2, ChevronUp, X
} from 'lucide-react';
...
const ServicePOS: React.FC<ServicePOSProps> = ({ type }) => {
  ...
  const CartPanel = () => (
    <div className="flex flex-col h-full bg-white">
        <div className="p-5 border-b border-gray-100 bg-gray-50 lg:rounded-t-2xl flex items-center justify-between">
            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl p-2 flex-1">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-white">
            {cart.length === 0 ? (
               ...
               cart.map(({item, qty}) => (
                  <div key={item.id} className="flex items-center justify-between p-3 bg-gray-50 border border-gray-100 rounded-xl shadow-sm">
        ...
        <div className="p-5 border-t border-gray-100 bg-gray-50 lg:rounded-b-2xl pb-safe">
    ...
    <div className="flex flex-col lg:flex-row h-[calc(100vh-140px)] gap-6 animate-in fade-in relative">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
           <div className="flex p-1 bg-gray-100 rounded-xl w-full md:w-auto">
              <button 
                ...
                className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-sm font-bold transition flex items-center justify-center gap-2 ${activeTab === 'services' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'}`}
              >
              ...
              <button 
                ...
                className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-sm font-bold transition flex items-center justify-center gap-2 ${activeTab === 'products' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'}`}
              >
           ...
           <div className="relative w-full md:w-64">
              ...
              <input 
                type="text" 
                placeholder="بحث..." 
                className="w-full bg-gray-50 border border-gray-200 rounded-xl py-2.5 pr-10 pl-4 text-base md:text-sm focus:outline-none focus:border-gray-400 transition"
                value={searchTerm}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4 overflow-y-auto pr-1 pb-20 lg:pb-4">
           {filteredItems.map(item => (
             <div 
               ...
               className={`cursor-pointer group p-4 rounded-2xl border transition-all hover:shadow-md flex flex-col items-center text-center active:scale-95 active:border-${themeColor}-500
      ...
      <div className="lg:hidden fixed bottom-4 left-4 right-4 z-40">
        <button 
          ...
          className={`w-full py-4 rounded-xl shadow-xl flex items-center justify-between px-6 text-white active:scale-95 transition-transform bg-${themeColor === 'yellow' ? 'yellow-500 text-black' : 'gray-900'}`}
        >
